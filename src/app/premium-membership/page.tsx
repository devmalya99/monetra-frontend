...
export default function PremiumMembershipPage() {
    ...
    useEffect(() => {
        const fetchMemberships = async () => {
            console.log("🚀 [PremiumMembershipPage] Step 1: Initiating fetch for premium memberships...");
            setLoading(true);
            try {
                const response = await axiosInstance.get('/premium/memberships');
                console.log("✅ [PremiumMembershipPage] Step 2: Successfully fetched memberships:", response.data);
                ...
                if (response.data.status === 'success') {
                    ...
                    console.log("✨ [PremiumMembershipPage] Step 3: Mapped plans configuration successfully");
                ...
                console.error("❌ [PremiumMembershipPage] Error: Failed to fetch premium memberships", error);
            } finally {
                setLoading(false);
                console.log("🏁 [PremiumMembershipPage] Step 4: Finished Premium Membership fetch process.");
            }
        };
        fetchMemberships();
    }, []);
    ...
    const handlePlanSelection = async (planId: string) => {
        try {
            console.log(`🚀 [PremiumMembershipPage] Step 1: Starting verification for Plan ID: ${planId}`);
            setVerifyingId(planId);
            const response = await axiosInstance.post('/premium/verify-order', {
                membership_id: planId
            });

            console.log('✅ [PremiumMembershipPage] Step 2: Response received from backend', response.data.data)
            const payment_id = response.data.data.razorpay_order_data.id;
            console.log('💳 [PremiumMembershipPage] payment_id:', payment_id)

            const options = {
                key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID,
                amount: 149900,
                currency: "INR",
                name: "Monetra Premium",
                description: "Purchase Monetra Premium Plan",
                order_id: payment_id,
                handler: async function (paymentResponse: any) {
                    console.log("💳 Payment successful, now verifying with backend...");
                    try {
                        // Trigger your verification API endpoint
                        ...
                        if (verifyRes.data.status === 'success') {
                            customToast.success("Verification successful! Enjoy your Premium features.");
                            setTimeout(() => {
                                window.location.href = '/dashboard';
                            }, 2000); // Give user time to see toast before redirecting
                        }
                    } catch (error) {
                        console.error("❌ Signature verification failed", error);
                        customToast.error("Payment was successful, but we couldn't verify it. Please contact support.");
                    }
                },
                prefill: {
                    name: "debmalyamazumdar99",
                    email: "debmalyamazumdar99@gmail.com",
                    contact: "9876543210"
                },
                theme: {
                    color: "#3399cc"
            ...
            console.error("❌ [PremiumMembershipPage] Verification failed", error);
        } finally {
            setVerifyingId(null);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col py-12 px-4 sm:px-6 lg:px-8 items-center">
            <div className="max-w-[850px] w-full bg-white shadow-2xl rounded-2xl flex flex-col overflow-hidden m-auto">
                {/* Header Area */}
                <div className="relative overflow-hidden bg-indigo-600 px-6 py-12 text-center shrink-0">
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                        <div className="w-16 h-16 bg-white/10 rounded-3xl border border-white/20 flex items-center justify-center mb-4 shadow-lg backdrop-blur-sm shadow-indigo-900/20">
                            <Crown className="w-8 h-8 text-amber-300" />
                        </div>
                        <h1 className="text-3xl font-extrabold text-white mb-2 text-center">
                            Upgrade to Monetra Premium
                        </h1>
                        <p className="text-indigo-100 font-medium text-base max-w-sm mx-auto text-center mt-2">
                ...
                <div className="p-6 md:p-10 bg-slate-50/50 flex-1">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-20 space-y-4">
                            ...
                            <p className="text-base font-medium text-slate-500">Loading premium plans...</p>
                        </div>
                    ) : (
                        <>
                            {/* Plans Grid */}
                            ...
                            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                                <div className="p-5 bg-slate-50 border-b border-slate-200 flex justify-between items-center text-sm font-bold text-slate-700">
                                    <div className="flex-1 px-2 text-base">Detailed Comparison</div>
